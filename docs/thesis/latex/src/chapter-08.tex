...
The latest benchmark exposes substantial runtime cost. The complete chained execution ran for 34 hours, 32 minutes, and 35 seconds. Phase 4 RAGAS evaluation consumed approximately 20 hours, 26 minutes, and 56 seconds, making it the main bottleneck. Mean generation latency was also materially higher for the RAG modes than for LLM only, with RAG with HyDE slower than RAG without HyDE.
...
The robustness artifacts also show a reproducibility boundary. The LLM-only baseline materially exercises seed and temperature variation, but the RAG API benchmark path remains deterministic under service-controlled generation defaults. Zero variance in RAG modes should therefore be interpreted as a configuration-path finding, not as evidence of stochastic robustness. Future work must expose request-level stochastic controls for RAG synthesis and HyDE generation before making robustness claims.
% evidence: evaluation_metrics/src/phases/phase3_generate.py
% evidence: biomed_knowledge_platform/src/biomed_platform/core/use_cases/ask.py
% evidence: evaluation_metrics/runs/old/june-07-valid-full-run-25-queries/20260605_123942_nogit/robustness

The repeated evaluator warning that one generation was returned where three were requested adds a second reproducibility boundary. The evaluation completed and produced usable artifacts, but evaluator configuration did not behave exactly as requested. That does not invalidate the benchmark; it changes how confidently the automated scores should be generalized. The result is a transparent limitation: the reported metrics are valid records of the executed pipeline, while future work should review evaluator configuration and calibrate automated judgments against expert review.
% evidence: evaluation_metrics/runs/old/june-07-valid-full-run-25-queries/evaluation.logs
