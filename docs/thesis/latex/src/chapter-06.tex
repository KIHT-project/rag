...
\section{CUDA-Aware Evaluation Implementation}

CUDA support in the thesis repository is implemented in the evaluation subsystem rather than in the core FastAPI service path. The evaluation CLI reads the \texttt{ragas.embeddings\_device} field from the active evaluation configuration and passes that value into the RAGAS execution phase, where the sentence-transformer evaluator embeddings are constructed for the configured device. In the branch "cuda-validation-metrics", commit \texttt{a1faf166} is the change point that introduced the CUDA-oriented evaluation profiles used for the RTX 5090 workstation configuration.
...
This implementation should be described narrowly. The verified CUDA path accelerates the evaluation-side embedding workload used during automated answer scoring; it does not by itself demonstrate that retrieval, indexing, or answer synthesis were all migrated to the same device. That boundary is important for methodological clarity because execution placement can change operational behavior without changing the meaning of the metric artifacts produced by the pipeline.
% evidence: evaluation_metrics/src/phases/phase4_ragas.py
